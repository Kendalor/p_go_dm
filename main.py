...
DATA_MINING_FILE= os.path.join('web','datamining.json')
DATA = {
    'pokemon':{},
    'pokestop':{},
    'gym':{}
...
def write_data_to_file():
    ...
    with open(DATA_MINING_FILE, 'w') as f:
        json.dump(DATA_MINED['pokemon'],f,indent=2)

def add_pokemon(pokeId, name, lat, lng, timestamp, timeleft):
    pokehash = '%s:%s:%s' % (lat, lng, pokeId)
    pokehash_DM ='%s:%s:%s:%s' % (lat, lng, pokeId, timestamp-timeleft+900)
    global MAX_TIMELEFT
    if pokehash_DM not in DATA_MINED['pokemon']:
        DATA_MINED['pokemon'][pokehash_DM] = {
            'id': pokeId,
            'name': name,
            'lat': lat,
            'lng': lng,
    ...
    if pokehash in DATA['pokemon']:
        ...
        if(DATA['pokemon'][pokehash]['timeleft'] > MAX_TIMELEFT and DATA['pokemon'][pokehash]['timeleft'] < 901):
            MAX_TIMELEFT=DATA['pokemon'][pokehash]['timeleft']
            print("MAX_TIMELEFT:"+str(MAX_TIMELEFT)+ " Seconds")
        else:
            print("MAX_TIMELEFT:" + str(MAX_TIMELEFT) + " Seconds")
